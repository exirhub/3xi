...
- **77 Python tests now pass** after adding installation-time gRPC options. New checks cover propagation to staged Xray, Nginx, Host records and exported URLs; exact source-database preservation, unchanged identities/counters/unmanaged inbounds, empty versus omitted authority, invalid/reserved paths, CLI/environment precedence and bootstrap forwarding. Generated cloud-init matches the shared bootstrap, passes YAML/Bash checks and remains below 16 KiB. The companion Exir panel has 12 code-generation/navigation tests plus production-browser checks for copy/download, defaults, form validation, Persian mobile and access restrictions. No new provider VM or live Cloudflare/VLESS session was provisioned.
