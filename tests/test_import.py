...
class ImportTests(unittest.TestCase):
    ...
    def test_preserves_identity_counters_and_source_bytes(self):
        ...
        with sqlite3.connect(self.source) as original, sqlite3.connect(self.folder/"render/x-ui.db") as staged:
            ...
            self.assertEqual(cfg["subURI"], "")
            self.assertEqual(cfg["webDomain"], "")
            self.assertEqual(cfg["subDomain"], "")
        self.assertEqual((self.folder/"render/etc/3xi/tls/origin.key").stat().st_mode & 0o777, 0o600)
        self.assertEqual((self.folder/"render/x-ui.db").stat().st_mode & 0o777, 0o600)
        self.assertEqual((self.folder/"render/var/www/3xi").stat().st_mode & 0o777, 0o755)
        self.assertEqual((self.folder/"render/var/www/3xi/index.html").stat().st_mode & 0o777, 0o644)
    ...
    def test_rejects_different_transport(self):
        ...
        self.edit_stream()
        with self.assertRaisesRegex(ConfigError, "Exactly one enabled local VLESS gRPC"):
            self.render()
