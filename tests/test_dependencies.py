...
failure = os.environ["THREEXI_TEST_APT_FAILURE"]
...
''')
            ...
            env = dict(os.environ, PATH=str(root) + os.pathsep + os.environ["PATH"],
                       THREEXI_TEST_APT_LOG=str(log), THREEXI_TEST_APT_FAILURE=failure)
            result = subprocess.run(
                ["bash", "-c", 'set -euo pipefail; source "$1"; threexi_install_dependencies; '
