...
if [[ -f /var/lib/3xi-bootstrap/source-commit.txt ]]; then
    printf 'Last successful bootstrap source commit: '
    cat /var/lib/3xi-bootstrap/source-commit.txt
fi
...
systemctl status x-ui.service threexi-nginx.service --no-pager -l || failed=1
ss -lntp '( sport = :80 or sport = :443 or sport = :10001 or sport = :8144 or sport = :2096 )' || failed=1
if [[ -x /usr/local/bin/3xi && -f /etc/3xi/installed.json ]]; then
    /usr/local/bin/3xi doctor || failed=1
else
    echo "The installed THREEXI command or installation state is missing."
    failed=1
fi
for log in /var/log/3xi-bootstrap.log /var/log/cloud-init-output.log; do
    if [[ -f "$log" ]]; then
        printf 'Recent output: %s\n' "$log"
        tail -n 40 "$log"
    fi
done
journalctl -u x-ui.service -u threexi-nginx.service -n 60 --no-pager -o cat || failed=1
exit "$failed"
