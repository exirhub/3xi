...
error_log {p("/var/log/3xi/nginx-error.log")} warn;
...
http {{
    ...
    access_log {p("/var/log/3xi/nginx-access.log")} threexi buffer=64k flush=5s;
    server_tokens off;
    sendfile on;
    tcp_nodelay on;
    gzip off;
...
def prepare(source: Path, output: Path, project: Path, *, domain="", advertised="",
            backend_port=10001, modern=True, ipv6=True, certificate_mode="embedded"):
    if output.exists():
